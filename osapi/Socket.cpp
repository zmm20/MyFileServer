...
int OS_TcpSocket::Accept(OS_TcpSocket* peer)
{
    ...
    return this->Accept(peer, &peerAddr);
}

int OS_TcpSocket::Accept(OS_TcpSocket* peer, OS_SockAddr* peerAddr)
{
    socklen_t addr_len = sizeof(sockaddr_in);
    socket_t peer_handle = socket_accept( hSock, (sockaddr*)peerAddr, &addr_len );
#ifdef _WIN32
...
int OS_TcpSocket::WaitBytes(void* buf, int count, /*out*/ int& realCount, int timeout)
{
    // 设置超时
    if(timeout > 0)
    {
        SetOpt_RecvTimeout(timeout);
    }
    ...
    {
        ret = Recv((char*)buf + realCount, count - realCount);
        if(ret <= 0)
        {
            break;
        }
...
int OS_UdpSocket::SetOpt_Broadcast(bool isOpen)
{
    int opt = isOpen ? 1: 0;
    socklen_t len = sizeof(opt);
    if (socket_setsockopt(hSock, SOL_SOCKET,SO_BROADCAST,(char*)&opt,len) < 0)
    {
        return -1;
    }
